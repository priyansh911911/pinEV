	...
	async function onSubmit(e: FormEvent<HTMLFormElement>) {
		...
		try {
			const verifyOtpRes = await verifyOtp({
				otp: { otp: otp, otpDetails: loginDetails?.otp },
				phone: loginDetails?.phone,
			});

			if (verifyOtpRes.err) {
				toast.error(verifyOtpRes?.message || "Invalid OTP");
				setError(verifyOtpRes?.message || "Invalid OTP");
				setIsLoading(false);
				return;
			}
				...
				<div className="flex flex-col gap-y-20 mt-20">
					...
					<form onSubmit={onSubmit} className="space-y-10">
						<div>
							...
							<p className="text-dark font-semibold text-base mt-2">
								We sent an OTP to{" "}
								<span className="font-bold text-lg">{loginDetails?.phone}</span>
							</p>
						</div>
